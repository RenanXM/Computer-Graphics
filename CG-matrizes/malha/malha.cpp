...
void Malha::calcN_e_R1xR2(Face *f, int i)
{
  /*
...
/*
  ...
  T =   0 0 1 tz
        0 0 0 1
*/
void Malha::transformacaoECorrecaoFaces(Matriz m, int i)
{
  ...
  for (Face *F: this->faces)
  {
    F->setPontos(prodMP(m, F->P1), prodMP(m, F->P2), prodMP(m, F->P3));
  }

  // Consertando os vetores
  this->calcFaces(i);
}
...
void Malha::translacao(double tx, double ty, double tz)
{
  ...
  this->transformacaoECorrecaoFaces(m, 1);
...
{
  Matriz m = Mespxy();
  this->transformacaoECorrecaoFaces(m, 2);
